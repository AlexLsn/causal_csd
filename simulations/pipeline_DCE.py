...
def calculate(model, X_function, estimator, detrend, realization):
    ...
    # Extract data for given realization
    ...
    if model in ('base', 'flatControl'):
        ...
        data = truncated_data_dict['truncated_data'][:, 1:] # remove control parameter time series

        indicator, residuals, _ = ind.pipeline_causalEE_weighted(causal_effects, data, estimator_class, 
                                                                        data_transform, n_points=150, time_windows=time_windows, 
                                                                        var_names=['Y'], detrend=detrend)
